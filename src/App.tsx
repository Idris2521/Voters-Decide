...
export default function App() {
  ...
  const handleFollow = async () => {
    ...
    try {
      const res = await fetch('/api/contests/official-contest/follow', {
        method: 'POST',
      });
      if (res.ok) {
        const data = await res.json();
  ...
  // 1. Fetch Contest & Contestants from server
  ...
  const fetchDeviceStatus = useCallback(async (slug = 'official-contest') => {
    ...
    try {
      const res = await fetch(`/api/contests/${slug}/device-status?token=${encodeURIComponent(deviceToken)}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleVoteSubmit = async (voterData: { fullName: string; whatsappNumber: string }) => {
    ...
    try {
      const res = await fetch(`/api/contests/${contest.slug}/vote`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          whatsappNumber: voterData.whatsappNumber,
        }),
      });

      const data: VoteSubmissionResult = await res.json();
      ...
      // Instruction 2: Once a voter cast their vote the follows numbers should add
      if (typeof data.followers_count === 'number') {
        setFollowersCount(data.followers_count);
      } else {
        setFollowersCount(prev => prev + 1);
