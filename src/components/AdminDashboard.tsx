...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [secretKey, setSecretKey] = useState(
    localStorage.getItem('vd_admin_token') || 'voters-decide-admin-2026'
  );
  ...
  const verifyAndLoad = async (tokenToTest: string) => {
    ...
    try {
      const res = await fetch('/api/admin/login', {
        ...
        body: JSON.stringify({ secretKey: tokenToTest }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setIsAuthenticated(true);
        localStorage.setItem('vd_admin_token', tokenToTest);
        loadAdminStats(tokenToTest);
      } else {
            ...
            <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
              ...
              <form onSubmit={handleAddContestant} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Bio / Project Summary</label>
                  <input
                    type="text"
                    placeholder="Short description of candidate"
                    value={newBio}
      ...
      {activeTab === 'supabase' && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs max-w-4xl space-y-6">
          <div className="flex items-center justify-between pb-4 border-b">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Schema file location: <code className="bg-slate-100 px-1 py-0.5 rounded text-slate-800">/supabase/schema.sql</code>
              </p>
            </div>
            <button
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3 text-xs text-slate-700 leading-relaxed">
            <h3 className="font-bold text-slate-900">Steps to Connect Your Supabase Project:</h3>
            <ol className="list-decimal pl-4 space-y-1.5">
              <li>
                In your Supabase project dashboard, open the <strong>SQL Editor</strong>.
              </li>
              <li>
                Open the file <code className="font-mono font-bold text-slate-800">supabase/schema.sql</code> in this project and paste its full contents into the Supabase SQL editor.
              </li>
              <li>
                Click <strong>Run</strong>. This will generate the <code className="font-mono">contests</code>, <code className="font-mono">contestants</code>, <code className="font-mono">participations</code>, and the atomic stored procedure <code className="font-mono">submit_vote</code>.
              </li>
              <li>
                Add your project credentials to your <code className="font-mono">.env</code>:
                <pre className="mt-2 p-3 bg-slate-900 text-slate-100 rounded-lg text-[11px] font-mono overflow-x-auto">
...
VITE_SUPABASE_ANON_KEY="eyJ..."</pre>
              ...
              <li>
                The backend automatically connects to Supabase and enables real-time subscriptions for all connected clients!
              </li>
            </ol>
          </div>
        </div>
