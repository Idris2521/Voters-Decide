...
const adminSecret = process.env.ADMIN_SECRET_KEY || 'voters-decide-admin-2026';
...
app.get('/api/config-status', async (req, res) => {
  ...
  if (supabase) {
    try {
      ...
      if (error) {
        supabaseStatus = 'configured_error';
  ...
  res.json({
    ...
    instructions: {
      step1: 'Configure SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in .env',
      step2: 'Run the schema in supabase/schema.sql in your Supabase SQL Editor',
      step3: 'Restart the dev server to synchronize with cloud database',
    }
  });
});
